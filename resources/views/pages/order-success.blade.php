<x-layouts.app>
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
        <div class="bg-white p-8 rounded-2xl border border-brand-green-100 shadow-sm">
            ...
            <div class="bg-brand-green-50/50 border border-brand-green-100 rounded-xl p-4 text-left max-w-sm mx-auto mb-8">
                <div class="flex justify-between mb-2">
                    <span class="text-xs font-bold text-brand-green-900">Order ID:</span>
                    <span class="text-xs text-brand-green-700">{{ $order->order_number }}</span>
                </div>
                <div class="flex justify-between mb-2">
                    <span class="text-xs font-bold text-brand-green-900">Amount Paid:</span>
                    <span class="text-xs text-brand-green-700">₹{{ number_format($order->total_amount, 2) }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-xs font-bold text-brand-green-900">Payment ID:</span>
                    <span class="text-xs text-brand-green-700">{{ $order->razorpay_payment_id }}</span>
                </div>
            </div>
